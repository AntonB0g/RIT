...
// modified by: S. Ciccarelli 02/19/2023 - fixed %error calculations and
// changed references to Lab #5
...
{
  Serial.begin(9600);
  Serial.println(F("Lab 5 S2 datatypes 2225"));
  Serial.println(F("\nEnter 'g' to go ....."));
  while (Serial.read() != 'g'); // spin until 'g' entry
  
  randomSeed(425);

  scale_factor = 10000.0; // start with 1.0
  xv = 1.0;
  ...
  Serial.print("Average percent error per calculation = ");
